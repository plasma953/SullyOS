...
type FilterTab = 'all' | 'recent' | 'manual';
...
const McpMemoryModal: React.FC<McpMemoryModalProps> = ({ charId, charName, onClose }) => {
    ...
    // 二次确认：'recent' = 清短期窗口，'manual' = 清手册类，'all' = 全部清空；null = 未在确认中
    const [confirming, setConfirming] = useState<null | 'recent' | 'manual' | 'all'>(null);
    ...
    const visible = useMemo(
        () => (tab === 'manual' ? entries.filter(e => e.persistent) : tab === 'recent' ? entries.filter(e => !e.persistent) : entries),
        [entries, tab],
    );

    const doClear = (scope: 'recent' | 'manual' | 'all') => {
            ...
            <div className="mcp-memory-shell px-4 sm:px-7">
                <header className="shrink-0 pt-[max(16px,env(safe-area-inset-top))] pb-3 border-b border-slate-900/10">
                    ...
                    <div className="mt-3 flex items-center gap-2">
                        {([
                            ...
                            { v: 'recent', label: `短期窗口 (${recentCount})` },
                    ...
                    {visible.length === 0 ? (
                        <div className="h-full min-h-64 grid place-items-center text-center px-8">
                            <div>
                                ...
                                <p className="mt-4 text-sm font-bold text-slate-500">{tab === 'manual' ? '没有长期保存的手册类结果' : tab === 'recent' ? '短期窗口还没有记录' : '还没有工具调用记录'}</p>
                                <p className="mt-1.5 text-xs leading-5 text-slate-400">
                                    {tab === 'all'
                                        ? '角色调用 MCP 工具后，结果会按轮次留档在这里，下一轮直接复用、不再重复调用。'
                                        : tab === 'recent'
                                            ? '短期窗口条目随对话轮次自动滚动淘汰，只保留最近若干轮，供模型直接复用。'
                ...
                <footer className="shrink-0 pb-[max(12px,env(safe-area-inset-bottom))] pt-2 border-t border-slate-900/10 flex items-center gap-2">
                    <button
                        ...
                        onClick={() => (confirming === 'recent' ? doClear('recent') : setConfirming('recent'))}
                        className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-colors disabled:opacity-40 ${confirming === 'recent' ? 'bg-violet-500 text-white' : 'bg-white/70 border border-slate-900/10 text-slate-600'}`}
                    >
                        {confirming === 'recent' ? '确认清短期' : `清空短期 (${recentCount})`}
                    </button>
                    <button
                        type="button"
                    ...
                    >
                        {confirming === 'manual' ? '确认清长期' : `清空长期 (${persistentCount})`}
                    </button>
                    <button
                        type="button"
                    ...
                    >
                        {confirming === 'all' ? '确认全清' : `全部清空 (${entries.length})`}
                    </button>
                </footer>
            </div>
        </div>
