...
import { buildAgentRelayRequest } from '../utils/agentRelayRequest';
  ...
  useEffect(() => {
      ...
      const patchedFetch = async (...args: [RequestInfo | URL, RequestInit?]) => {
          ...
          if (urlStr.includes('/chat/completions')) {
              ...
              // ── VPS 主代理中转（预设联动，可选）──────────────────────────────
              // 改写逻辑与 utils/externalRequest 共用（utils/agentRelayRequest.ts）：
              // 设置里填了 agentUrl（os_api_config.agentUrl）后，/chat/completions 改发
              // ${agentUrl}/agent/v1/chat/completions：原供应商 baseUrl/apiKey/model
              ...
              const relayed = buildAgentRelayRequest(urlStr, sendArgs[1] as RequestInit, readAgentRoutingConfig());
              if (relayed) {
                  sendArgs = [relayed.url, relayed.init];
              }
          }
