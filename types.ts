...
export interface OSTheme {
  ...
  /** 桌面端布局模式：auto = 按窗口尺寸自动（宽>=1024 且高>=600 用电脑版）；on/off = 强制电脑版/手机版。默认 auto（undefined 视为 auto）。 */
  desktopMode?: 'auto' | 'on' | 'off';
  /** 电脑版布局背景：blur = 当前壁纸模糊放大（默认）；color = 壁纸主色调纯色。 */
...
export interface StudyChapter {
    ...
    content?: string;
    /** EPUB：blobref 改写后的章节 HTML（清洗后的 fragment）。 */
    rawHtml?: string;
    /** AI 章节总结缓存（按需生成）。 */
    aiSummary?: string;
    /** AI 总结生成状态（驱动侧滑面板三态）。 */
    aiSummaryState?: 'idle' | 'loading' | 'done' | 'error';
    /** EPUB：本章纯文本全文（AI 总结素材；summary 仅存 120 字摘要展示用）。 */
    plainText?: string;
